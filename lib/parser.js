...
function Parser (config) {
  ...
  this._config = config
  this._client = cache.createClient(config)
...
Parser.SERVICES = [
  {
    ...
    provider: 'youtube',
    pattern: /(https?:\/\/)?(www.)?(youtube\.com|youtu\.be|youtube-nocookie\.com)\/(?:embed\/|v\/|shorts\/|watch\?v=|watch\?list=(.*)&v=|watch\?(.*[^&]&)v=)?((\w|-){11})(&list=(\w+)&?)?/,
    method: 'loadYoutube',
    index: 6
  },
  {
...
Parser.prototype.load = function (cb, fail, cacheId) {
  ...
  this._client.get(cacheId, function (err, item) {
    if (err || item) {
      ...
      fail(function (err, item) {
        self._client.set(cacheId, typeof err === 'string' ? err : item, self._config.ttl, function (err) {
          if (err) {
            self.emit('error', err)
          }
        })
...
Parser.prototype.requestFacebook = function (cb, id) {
  ...
  var _requestFacebook = function () {
    ...
    FB.napi(id, { fields: ['format', 'description', 'created_time', 'from', 'published', 'title', 'source'] }, function (err, res) {
      if (err) {
      ...
      if (res.source) {
        self.loadMeta(function (err, meta) {
          ...
          video.duration = Math.round(meta.format.duration)
          cb(null, video)
