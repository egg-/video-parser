...
Parser.prototype.requestNavertvcast = function(cb, id) {
	...
	return this.requestNavertvcastMeta(function(err, meta) {
		if (err) {
			return cb(err);
		}
		...
		// load outkey and extra meta
		async.parallel({
			outkey: function(cb) {
				self.requestNavertvcastOutkey(cb, meta.video.id);
			},
			extra: function(cb) {
				self.requestNavertvcastExtra(cb, meta.video.id, meta.video.key);
			}
		}, function(err, result) {
			if (err) {
				return cb(err);
			...
			video.duration = result.extra.duration;
			video.thumb_url = result.extra.image;
			video.details.author = {
				id: result.extra.user.id,
				title: result.extra.user.name
			};
			video.details.naver.outkey = result.outkey;
...
Parser.prototype.requestNavertvcastMeta = function(cb, id) {
	...
	request(url, function(err, res, body) {
		...
		// extract outkey
		var regexp = /nhn.rmcnmv.RMCVideoPlayer\("(\w+)", "(\w+)"/gim;
