...
Parser.prototype.requestNavertvcast = function(cb, id) {
	...
	async.parallel({
		meta: function(cb) {
			og('http://tvcast.naver.com/v/' + id, cb, {
				overall: true,
				namespaces: [
					'og', 'naver'
				]
		...
		extra: function(cb) {
			self.requestNavertvcastExtra(cb, id);
		}
	}, function(err, result) {
		if (err) {
			return cb(err);
		...
		var meta = result.meta.og;
		var naver = result.meta.naver;
		var channel = result.extra.channel;
		var trackingData = result.extra.trackingData;
		...
		var video = {
			...
			thumb_url: meta.image.url,
			duration: trackingData ? parseFloat(trackingData.playtime) : 0,
			ctime: (trackingData ? moment(trackingData.broaddate, 'YYYYMMDD').utcOffset(9) : moment.unix(0)).format(),
			ratings: {},
			details: {
				definition: '',
				author: channel ? {
		...
		// add share key
		self.requestNavertvcastShare(function(err, key) {
			if (err) {
				return cb(err);
			}

			video.details.naver.key = key;
...
Parser.prototype.requestNavertvcastExtra = function(cb, id) {
	var url = 'http://tvcast.naver.com/v/' + id;

	var extractTrackingData = function(str) {
		var patternTracking = /(.*)rmcPlayer.properties.externalTrackingData\s+=\s+(.*);/gim;
		var matches = patternTracking.exec(str);
		if (matches) {
			var query = JSON.parse(matches[2]).params.adlink;
	...
	request(url, function(err, res, body) {
		...
		var channel = $('.video_ch h2 a', '#expandChannelInfo');
		var trackingData = extractTrackingData(body);

		if (channel) {
			var link = channel.attr('href');
			var title = channel.text();
...
Parser.prototype.requestNavertvcastShare = function(cb, vid) {
